...
@section('action-btn')
    <div class="float-end">
        ...
        @can('create invoice')
            <a href="{{ route('car-invoice.create') }}" class="btn btn-sm btn-primary" data-bs-toggle="tooltip" title="{{__('Create')}}">
                                ...
                                @foreach ($invoices as $invoice)
                                    <tr>
                                        <td class="Id">
                                            ...
                                            @else
                                                <a href="{{ route('car-invoice.show', \Crypt::encrypt($invoice->id)) }}" class="btn btn-outline-primary">{{ AUth::user()->invoiceNumberFormat($invoice->invoice_id) }}</a>
                                            @endif
                                        </td>
                                        <td>{{ $invoice?->invoiceProduct?->inventory()?->name ?? ''}}</td>
                                        @if (!\Auth::guard('customer')->check())
...
{{--                                                        </div>--}}
                                                    ...
                                                    @can('show invoice')
                                                        ...
                                                        @else
                                                                <div class="action-btn bg-warning ms-2">
                                                                    <a href="{{ route('car-invoice.show', \Crypt::encrypt($invoice->id)) }}"
                                                                        class="mx-3 btn btn-sm align-items-center" data-bs-toggle="tooltip" title="Show "
                                                                        data-original-title="{{ __('Detail') }}">
                                                                        <i class="ti ti-eye text-white"></i>
                                                                    </a>
                                                    ...
                                                    @can('edit invoice')
                                                            <div class="action-btn bg-info ms-2">
                                                                <a href="{{ route('car-invoice.edit', \Crypt::encrypt($invoice->id)) }}"
                                                    ...
                                                    @can('delete invoice')
                                                            <div class="action-btn bg-danger ms-2">
                                                                {!! Form::open(['method' => 'DELETE', 'route' => ['car-invoice.destroy', $invoice->id], 'id' => 'delete-form-' . $invoice->id]) !!}
