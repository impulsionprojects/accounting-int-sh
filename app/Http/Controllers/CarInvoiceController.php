...
class CarInvoiceController extends Controller
{
    ...
    public function index( Request $request )
    {
        if ( \Auth::user()->can('manage invoice') ) {
            ...
            $query = Invoice::with(['invoiceProduct'])->where('income_type','car_invoice')->where('created_by', '=', \Auth::user()->creatorId())->orderBy('id','desc');
            ...
            return view('car_invoice.index', compact('invoices', 'customer', 'status'));
        } else {
    ...
    public function create ()
    {
        if ( \Auth::user()->can('create invoice') ) {
            ...
            return view('car_invoice.create', compact('customers', 'invoice_number', 'product_services', 'category', 'customFields', 'inventory'));
        } else {
    ...
    public function customer( Request $request )
    {
        ...
        return view('car_invoice.customer_detail', compact('customer'));
    }
    ...
    public function store(Request $request)
    {
        if (\Auth::user()->can('create invoice')) {
            ...
            try {
                ...
                $invoice->income_type = "car_invoice";
                ...
                for ($i = 0; $i < count($products); $i++) {
                    ...
                    // $journal['totalTax'] += !empty($products[$i]['tax']) ? Utility::taxCalculation($products[$i]) : 0;
                    // $journal['totalDiscount'] += isset($products[$i]['discount']) ? floatval($products[$i]['discount']) : 0;

                    // $accountId = ProductService::find($products[$i]['item'])->account; // Get the revenue account ID

                    // $journal['items'][] = [ 'price' => $totalPrice, 'account' => $accountId ];
                ...
                // EntryJournal::journalMechanism(JournalEntryType::INVOICE, $invoice->id, $journal);
                ...
                return redirect()->route('car-invoice.index', $invoice->id)->with('success', __('Invoice successfully created.'));
            } catch (\Exception $e) {
                DB::rollBack();
    ...
    public function edit( $ids )
    {
        if ( \Auth::user()->can('edit invoice') ) {
            ...
            $invoice = Invoice::where('id',$id)->where('income_type','car_invoice')->first();
            ...
            return view('car_invoice.edit', compact('customers', 'product_services', 'invoice', 'invoice_number', 'category', 'customFields','inventory'));
        } else {
            return response()->json([ 'error' => __('Permission denied.') ], 401);
        }
    }
...
//                $invoice->category_id = $request->category_id;
                ...
                CustomField::saveData($invoice, $request->customField);
                 ...
                 $invoiceProduct->quantity = 1;
                 $invoiceProduct->product_id = $product[0]['item'];
                //  $invoiceProduct->tax = $product[0]['tax'];
                 $invoiceProduct->discount = $product[0]['discount'];
                 $invoiceProduct->price = $product[0]['price'];
                 $invoiceProduct->description = $product[0]['description'];
                 $invoiceProduct->save();
                // For Journal Entry
...
//                         $invoiceProduct = new InvoiceProduct();
//                         $invoiceProduct->invoice_id = $invoice->id;
//                         Utility::total_quantity('minus', $product['quantity'], $product['item_id']);
//                     } else {
...
//                         $invoiceProduct->product_id = $product['item_id'];
//                     }
                    ...
                    // $invoiceProduct->save();
//                     // Utility::total_quantity('plus',$product['quantity'],$invoiceProduct->product_id);

//                     if ( $product['id'] > 0 ) {
//                         Utility::total_quantity('plus', $product['quantity'], $invoiceProduct->product_id);
//                     } else {
//                         Utility::total_quantity('plus', $product['quantity'], $product['item_id']);
//                     }
...
//                     $description = $product['quantity'] . '  ' . __(' quantity sold in invoice') . ' ' . \Auth::user()->invoiceNumberFormat($invoice->invoice_id);
//                     if ( empty($product['id']) ) {
//                         Utility::addProductStock($product['item_id'], $product['quantity'], $type, $description, $type_id);
//                     }
                ...
                return redirect()->route('car-invoice.index')->with('success', __('Invoice successfully updated.'));
            } else {
                return redirect()->back()->with('error', __('Permission denied.'));
            }
        } else {
    ...
    public function show( $ids )
    {
        ...
        if ( \Auth::user()->can('show invoice') ) {
            ...
            if ( $invoice->created_by == \Auth::user()->creatorId() ) {
                ...
                return view('car_invoice.view', compact('invoice', 'customer', 'iteams', 'invoicePayment', 'customFields'));
            } else {
                return redirect()->back()->with('error', __('Permission denied.'));
            }
        } else {
    ...
    public function destroy($id)
    {
        ...
        if ( \Auth::user()->can('delete invoice') ) {
            ...
            if ( $invoice->created_by == \Auth::user()->creatorId() ) {
                ...
                return redirect()->route('car-invoice.index')->with('success', __('Invoice successfully deleted.'));
            } else {
                return redirect()->back()->with('error', __('Permission denied.'));
            }
        } else {
    ...
    public function customerInvoiceShow( $id )
    {
        ...
        if ( \Auth::user()->can('show invoice') ) {
            ...
            $invoice = Invoice::where('id',$invoice_id)->where('income_type','car_invoice')->first();
    ...
    public function createPayment( Request $request, $invoice_id )
    {
        ...
        if ( \Auth::user()->can('create payment invoice') ) {
            ...
            try {
                ...
                EntryJournal::invoicePaymentJournal(JournalEntryType::INVOICE_PAYMENT, $invoicePayment->id, $request->account_id, $request->amount);
    ...
    public function inventory( Request $request )
    {
        ...
        $data['product'] = $product = Inventory::find($request->product_id);
        // $data['unit'] = ( !empty($product->unit()) ) ? $product->unit()->name : '';
        // $data['taxRate'] = $taxRate = !empty($product->tax_id) ? $product->taxRate($product->tax_id) : 0;
        // $data['taxes'] = !empty($product->tax_id) ? $product->tax($product->tax_id) : 0;
        $salePrice = $product->sale_price;
